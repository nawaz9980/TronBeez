  ...
  const handleWithdraw = async (chatId, tgId) => {
    try {
      const [users] = await db.query(
        `SELECT wallet_address, balance, withdrawn, max_withdrawals, username FROM users WHERE tg_id = ?`,
        [tgId]
      );
      const user = users[0];
      ...
      const listener = async (resp) => {
        try {
          ...
          const [[fresh]] = await db.query(
            `SELECT wallet_address, balance, max_withdrawals, username FROM users WHERE tg_id = ? LIMIT 1`,
            [tgId]
          );
          ...
          const txnId = data.payout_id || "N/A"; // ✅ Transaction ID from FaucetPay

          // Deduct balance & increment withdrawn
          await db.query(
            `UPDATE users 
             SET balance = balance - ?, 
                 withdrawn = withdrawn + ?,
          ...
          const logChannel = process.env.LOG_CHANNEL_ID; // set in .env
          if (logChannel) {
                ...
                reply_markup: {
                  inline_keyboard: [
                    [
                      {
                        text: "🤖 View Bot",
                        url: `https://t.me/${process.env.BOT_USERNAME}`, // ✅ from .env
                      },
                    ],
                  ],
                },
